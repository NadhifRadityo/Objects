...
public class OptimizedQueueList<E> implements DeadableObject, Iterable<E> {
	private static final long[] $S_AFIELD_SynchronizedObjectLongMap = EclipseCollectionsUtils.$S_AFIELD_Synchronized$(SynchronizedObjectLongMap.class);
	private static final long[] $HM_AFIELD_ObjectLongHashMap = EclipseCollectionsUtils.$HM_AFIELD_$HashMap(ObjectLongHashMap.class);
	private static final Object FIELD_VAL_ObjectLongHashMap_NULL_KEY = EclipseCollectionsUtils.getSentinelMap_NULL_KEY(ObjectLongHashMap.class);
	private static final Object FIELD_VAL_ObjectLongHashMap_REMOVED_KEY = EclipseCollectionsUtils.getSentinelMap_REMOVED_KEY(ObjectLongHashMap.class);
	...
	public Map.Entry<E, Long>[] getMap() {
		MutableObjectLongMap<E> sourceMap = (MutableObjectLongMap<E>) EclipseCollectionsUtils.getSynchronizedSourceMap(this.map, $S_AFIELD_SynchronizedObjectLongMap);
		Map.Entry<E, Long>[] result = new Map.Entry[sourceMap.size()]; int j = 0;
		E[] keys = (E[]) EclipseCollectionsUtils.getKeys(sourceMap, $HM_AFIELD_ObjectLongHashMap);
		long[] values = (long[]) EclipseCollectionsUtils.getValues(sourceMap, $HM_AFIELD_ObjectLongHashMap);
		for(int i = 0; i < keys.length; i++) { E key = keys[i]; long value = values[i];
			if(key == null || key == FIELD_VAL_ObjectLongHashMap_NULL_KEY || key == FIELD_VAL_ObjectLongHashMap_REMOVED_KEY) continue;
	...
	private synchronized ObjectLongPairImpl<E> getSortedEntry(boolean fromOldest) {
		pair.setOne(FIELD_VAL_ObjectLongHashMap_REMOVED_KEY); pair.setTwo(Long.MAX_VALUE);
		MutableObjectLongMap<E> sourceMap = (MutableObjectLongMap<E>) EclipseCollectionsUtils.getSynchronizedSourceMap(this.map, $S_AFIELD_SynchronizedObjectLongMap);
		E[] keys = (E[]) EclipseCollectionsUtils.getKeys(sourceMap, $HM_AFIELD_ObjectLongHashMap);
		long[] values = (long[]) EclipseCollectionsUtils.getValues(sourceMap, $HM_AFIELD_ObjectLongHashMap);
		for(int i = 0; i < keys.length; i++) { E key = keys[i]; long value = values[i];
			if(key == null || key == FIELD_VAL_ObjectLongHashMap_REMOVED_KEY) continue;
		...
		} if(pair.getOne() == FIELD_VAL_ObjectLongHashMap_NULL_KEY) pair.setOne(null);
		return pair.getOne() != FIELD_VAL_ObjectLongHashMap_REMOVED_KEY ? (ObjectLongPairImpl<E>) pair : null;
	}
