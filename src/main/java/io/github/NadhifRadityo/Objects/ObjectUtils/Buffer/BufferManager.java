package io.github.NadhifRadityo.Objects.ObjectUtils.Buffer;

import io.github.NadhifRadityo.Objects.ObjectUtils.Buffer.BufferAlgorithm.BufferAllocationAlgorithm;
...
//	}
	...
	public BufferManager(BufferAllocationAlgorithm allocationAlgorithm, ByteBuffer buffer) {
		...
		this.buffer = buffer;
		this.address = BufferUtils.__getAddress(buffer);
		this.totalSize = buffer.capacity();
...
//		bufferManagers.add(this);
	...
	public boolean isPartOfBuffer(long address, int capacity) { return address >= this.address && address + capacity <= this.address + buffer.capacity(); }
	public boolean isPartOfBuffer(Buffer buffer) { return isPartOfBuffer(BufferUtils.__getAddress(buffer), buffer.capacity() * BufferUtils.getElementSize(buffer)); }

	public synchronized long _allocate(int size) {
//		return BufferUtils.createByteBuffer(size);
		if(size <= 0) return address;
		int start = allocationAlgorithm.allocate(this, map, size, cache);
...
//			System.out.println("BAD SECTOR (" + i + ")"); buffer.put(i, (byte) 0); }
	...
	public synchronized void _deallocate(long address) {
		...
		int end = localAddress + Math.abs(map[index]);
		for(int i = localAddress; i < end; i++) this.buffer.put(i, (byte) 0);
...
//		System.out.println(Thread.currentThread() + ": Deallocate -> Address:" + address + ", local address:" + localAddress + ", DUMP: " + ArrayUtils.deepToString(map));
	...
	public synchronized ByteBuffer allocate(int size) { return BufferUtils.__pointTo(_allocate(size), size); }
	public synchronized void deallocate(Buffer buffer) { _deallocate(BufferUtils.__getAddress(buffer)); }
	public synchronized boolean canAllocate(int size) { return allocationAlgorithm.canAllocate(this, map, size, cache); }
}
