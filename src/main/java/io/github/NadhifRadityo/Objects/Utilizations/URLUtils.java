...
public class URLUtils {
	...
	public static String getSourceCode(String url, Charset charset) throws IOException {
		...
		curl.setCustomHandler((ReferencedCallback.PVoidReferencedCallback) (args) -> {
		...
		curl.setOnConnect((args) -> ExceptionUtils.doSilentThrowsReferencedCallback(ExceptionUtils.silentException, (_args) -> StreamUtils.toString(((URLConnection) args[0]).getInputStream(), charset)));
	...
	public static class CURL implements DeadableObject {
		...
		protected ReferencedCallback.VoidReferencedCallback customHandler;
		protected ReferencedCallback.ObjectReferencedCallback onConnect;
		...
		public void setCustomHandler(ReferencedCallback.VoidReferencedCallback customHandler) { this.customHandler = customHandler; }
		public void setOnConnect(ReferencedCallback.ObjectReferencedCallback onConnect) { this.onConnect = onConnect; }
		...
		public ReferencedCallback.VoidReferencedCallback getCustomHandler() { return customHandler; }
		public ReferencedCallback.ObjectReferencedCallback getOnConnect() { return onConnect; }
		...
		public Object build(Proxy proxy, Charset charset) throws IOException {
			...
			if(requestMethod == RequestMethod.POST && posts.size() > 0) {
				...
				for(Map.Entry<String, String> entry : posts.entrySet())
					postJoiner.add(URLEncoder.encode(entry.getKey(), charset.name()) + "=" +
								   URLEncoder.encode(entry.getValue(), charset.name()));
				postByte = postJoiner.toString().getBytes(StandardCharsets.UTF_8);
			}
			...
			if(customHandler != null) customHandler.get(urlConnection, urlObject, isHttps, this);
			urlConnection.connect();
			if(postByte != null) try(OutputStream outputStream = urlConnection.getOutputStream()) { outputStream.write(postByte); }
			if(onConnect == null) return urlConnection; try {
				return onConnect.get(urlConnection, urlObject, isHttps, this); } finally {
