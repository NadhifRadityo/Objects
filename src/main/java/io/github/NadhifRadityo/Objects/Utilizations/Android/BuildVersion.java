	...
	static {
		try {
			for(Field field : BuildVersion.class.getFields()) {
				...
				name = StringUtils.capitaliseEachWords(name.toLowerCase()).replaceAll("Mr", "MR");
				
				String[] underscores = field.getName().split("_");
				if(underscores.length >= 3 && NumberUtils.isNumber(underscores[underscores.length - 2]) && NumberUtils.isNumber(underscores[underscores.length - 1])) {
					name = name.replaceAll(underscores[underscores.length - 2] + " " + underscores[underscores.length - 1], "");
					name += underscores[underscores.length - 2] + "." + underscores[underscores.length - 1];
				}
				buildVersions.put(name, (Integer) field.get(null));
			}
