...
public class JCEFMessageRouters implements DeadableObject {
	...
	public CefClient getClient() { assertNotDead(); return client; }
	public CefBrowser getBrowser() { assertNotDead(); return browser; }
	public CefMessageRouter getRouter() { assertNotDead(); return router; }
	...
	protected void attachListener() {
		router.addHandler(new CefMessageRouterHandlerAdapter() {
			...
			@Override public boolean onQuery(CefBrowser browser, CefFrame frame, long query_id, String request,
					boolean persistent, CefQueryCallback callback) { ExceptionUtils.doSilentThrowsRunnable(exceptionHandler, () -> {
				...
				if(type == MessageType.ERROR) exceptionHandler.onExceptionOccurred(new Exception(new JSONObject(data).getString("stack")));
					...
					} String finalHash = SecurityUtils.getSHA256String(StringUtils.mergeCross(shuffle));
