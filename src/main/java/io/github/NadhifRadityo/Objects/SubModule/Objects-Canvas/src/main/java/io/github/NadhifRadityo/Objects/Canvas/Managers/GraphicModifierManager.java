...
public class GraphicModifierManager extends ImplementSpriteManager {
	...
	public Map.Entry<CustomGraphicModifier, Integer>[] getModifiers() { return modifiers.getMap(); }
	...
	protected class GraphicModifierSprite extends ImplementSprite implements OverrideGraphic, CanvasPanel.SpriteChecker {
		Graphics graphics;
	...
	protected class GraphicModifierChecker implements CanvasPanel.SpriteChecker {
		Graphics oldGraphic;
		@Override public Graphics beforeDraw(Sprite sprite, Graphics g) { this.oldGraphic = g;
			Set<CustomGraphicModifier> modifiers = GraphicModifierManager.this.modifiers.get();
			Graphics graphic;
			for(CustomGraphicModifier modifier : modifiers) {
				if((modifier instanceof CanvasPanel.SpriteChecker) && (modifier.sprite.size() == 0 ||
						ListUtils.containsEquals(modifier.sprite, sprite))) {
					graphic = ((CanvasPanel.SpriteChecker) modifier).beforeDraw(sprite, g);
					if(graphic != null) g = graphic;
		...
		@Override public Graphics afterDraw(Sprite sprite, Graphics g) {
			...
			for(CustomGraphicModifier modifier : modifiers) {
				...
				graphic = ((CanvasPanel.SpriteChecker) modifier).afterDraw(sprite, g);
				if(graphic != null) g = graphic;
	...
	@Override protected void init(CanvasPanel canvas) {
		super.init(canvas); canvas.addChecker((CanvasPanel.SpriteChecker) sprite, priority);
		if(!isApplyToGraphic()) canvas = toApply;
		canvas.addChecker(checker, priority);
	}
	@Override protected void uninit(CanvasPanel canvas) {
		super.uninit(canvas); canvas.removeChecker((CanvasPanel.SpriteChecker) sprite);
		if(!isApplyToGraphic()) canvas = toApply;
